...
**Hard:** no result is citable until env, precision, seeds, and inputs are pinned and recorded. **Adaptable:** the capture mechanism and format.
...
4. **Precision** — float32 vs float64 and the flag that set it (`jax.config.update("jax_enable_x64", True)`). Silent float32 is a top cause of "irreproducible" drift. Record the float64 flag **even when a package sets it implicitly at import** (e.g. progenax enables it via `jaxstro.jaxconfig.enable_high_precision()`), so the import-time default is visible in `env.json` rather than assumed.
...
- Prefer machine-captured over hand-typed: emit the six fields programmatically (a `env.json` / lockfile) so they can't drift from reality — e.g. `uv pip freeze` for deps, `jax.devices()` + `jax.print_environment_info()` for interpreter/accelerator/backend.
