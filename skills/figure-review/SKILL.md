...
A figure can fail three independent ways, and a review that blurs them misses at least one: it can **communicate badly** (design), **misrepresent the data** (faithfulness), or be **honestly drawn and still misread** (interpretation). Name the mode you are in; run more than one only when asked or when a submission-bound figure warrants it.

## Mode 1 — Design (does it make the point?)
- **Chart-type fit** → the geometry matches the data and the claim: trend → line; distribution → histogram/ECDF/violin; correlation → scatter; field → image/contour/quiver.
- **Every channel carries information** → position for the most important variable; drop decorative color/size; don't encode one variable twice.
- **Declutter** → direct labels over a detached legend when traces are few; no chartjunk.
- **3-second test** → the takeaway survives a glance, or the encoding/annotation is failing.
- **Generative** (data + claim, no figure yet) → propose 2-3 designs, each with the encoding, why it serves *this* claim, and what it hides. Small multiples for sweeps; residual/ratio panels for model vs data.
- Anti-patterns: dual y-axes that manufacture a correlation; stacked areas for non-additive quantities; one overplotted panel where small multiples would separate regimes.

## Mode 2 — Faithfulness (is what's plotted what should be plotted?)
Read the **rendered image**, not only the code.
- **Data match** → spot-check a few drawn values against the source arrays; catch the wrong column, a swapped series, a stale file.
- **Axes honesty** → scale, range, and units; truncated or zoomed ranges that create or hide an effect; a log axis concealing sign changes.
- **Silent omissions** → clipped ranges, dropped NaNs/outliers, downsampling, a fit drawn outside its valid domain.
- **The claimed effect is visible** → and not an artifact of binning, smoothing, or marker size.
- Output per issue: what the figure shows · what the data says · the discrepancy. Verdict: faithful / faithful-with-caveats / misleading.

## Mode 3 — Interpretation (what does it license you to conclude?)
- **Shown vs inferred** → a trend seen by eye is a hypothesis until checked against the numbers and their uncertainty; curves that "look different" may overlap within error bars the figure omits.
