...
## Checklist
- **A reported error below ~1e-7 in JAX needs x64 evidence** → float32 resolution is ~1.2e-7, so a claimed `|ΔE/E| = 3e-12` from float32 JAX is an artifact. The `inference_precision_gate.sh` Stop hook blocks such a claim unless `jax_enable_x64` appears in the message, the turn, or the repo.
