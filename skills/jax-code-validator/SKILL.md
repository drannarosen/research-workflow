...
Most JAX mistakes are loud: a traced value used as a Python bool raises. Review effort belongs on the
ones that are silent: they compile, run, and return a plausible wrong number. Behavior below was
verified against JAX 0.11 (2026-09). JAX semantics change between versions, so a claim about JAX
behavior that is not in these tables gets a three-line check before you assert it.
...
| Leaked tracer (stored on `self`, a global, or a closure cache during tracing) | `UnexpectedTracerError` later, or a stale traced value reused across calls. | Return state; don't stash intermediates outside the function. |
...
Findings as a short list, each with `file:line`, the trap, whether it is silent or loud, and the fix.
Lead with silent traps.
