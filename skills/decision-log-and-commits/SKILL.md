...
> **Never hide a scientific behavior change inside a "refactor", "cleanup", or "tidy" commit.** A change that moves equation/source/boundary ownership, or shifts a measured result, gets its own commit with the supporting artifact path in the message (→ `run-reproducibility`). This is the hard rule.
