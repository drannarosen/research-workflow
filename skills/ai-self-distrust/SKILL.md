...
- **Plausible-but-wrong math** → a derivation with a dropped factor or flipped sign. Route to `derivation-before-implementation` + `adversarial-result-check`; never accept symbolic work because it "looks standard."
...
- `derivation-before-implementation` / `adversarial-result-check` — for AI-produced math and numbers.
