...
argument-hint: <reference, e.g. mesa | a paper's Table 2>
---
Use the `reference-parity-audit` skill to plan and run a parity check of our implementation against: $ARGUMENTS

Steps:
This is the late-stage validation milestone (see `verification-gate`, *Validation stages*); run it when a result is heading for publication or release, not as a precondition for ongoing development.

1. If the reference matches a shipped lens (`mesa`), load that lens for its domain-specific gotchas before designing the comparison.
