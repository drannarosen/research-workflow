...
### Fixed (2026-09-23 adversarial review — hooks that missed their headline case)
- **`test_integrity.sh`: deleting every assertion was never flagged.** `grep -c … || echo 0` produced `"0\n0"` on zero matches, the integer comparison errored silently, and the all-asserts-removed edit passed. Dropped the fallback (+2 hook tests).
- **`no_secrets_in_git.sh`: secrets staged by the same command, or already staged under `-a`/`--amend`, were not scanned.** PreToolUse runs before `git add … && git commit`, so the index was still empty; and the `-a` heuristic (which also matched `--amend` and `-a` inside a message) swapped the index scan for an unstaged-only one. The index is now always scanned; when the command itself stages, unstaged tracked changes and untracked files (≤200, ≤256 KiB each) are scanned too (+8 hook tests).
- **`evidence_gate.sh`: a failing test run counted as evidence for "all tests pass".** New contradiction check: a tests-pass claim is flagged when the last test summary among this turn's Bash results reports failures, unless the message acknowledges them (+3 hook tests).
- **`myst_docs_hygiene.sh` fired in every repo with a `docs/` folder.** Plans written to `docs/plans/` and Sphinx pages (where `{toctree}` is valid) raised permission prompts, and an Edit starting with a `---` horizontal rule was read as frontmatter. The gate now acts only when a `myst.yml`/`myst.yaml` sits in the page's directory or up to 6 parents, and on Edit treats a leading `---` as frontmatter only when the replaced text is a frontmatter block (+5 hook tests).
- **The hook suite now records what the hooks still miss.** A strict-xfail `gap` helper encodes the 22 still-open probes from the review (e.g. `git reset --hard` not gated, pytest.approx loosening, "stable" read as a Table citation, stop_hook_active ignored, G = 6.674e-8 cgs flagged as a float32 error). Open gaps print `GAP` without failing; a gap that starts passing fails the suite until it is promoted to a `check`.
- **`ownership-and-structure` filed analytic checks under milestone-only validation**, contradicting `verification-gate`'s *Validation stages*. The *independent validation* label now separates an analytic solution or limit (development stage, as early as possible) from a published reference or separate code (end stage).
- **A routing pointer led nowhere, and the dangling-reference check hid it.** `myst-expert` pointed Quarto work to `quarto-expert`, which exists in no plugin; `checks.sh` allowlisted it along with three other names referenced nowhere. Dropped the pointer, pruned the allowlist to targets that exist, marked `docs-writing-voice`'s grant hand-off as the grant-writing plugin's, and declared every external hand-off in a README *Companion plugins and skills* table.
- **CI runs on macOS too**, with the system `/bin/bash` 3.2 pinned first on PATH, so BSD awk/grep and bash-3.2 behaviour are tested where researchers run the hooks. shellcheck stays Linux-only.
...
- **Second consolidation (54 → 43, ADR-0015):** `verification-gate` absorbs `evidence-first-execution`; `uncertainty-reporting-gate` absorbs `seed-and-stochasticity`; new `run-reproducibility` (experiment tracking, artifact bundle, environment contract, cluster jobs); new `provenance` (constants + data); `research-release-checklist` absorbs `software-citation`; new `hypothesis-and-test-design` (brainstorming + discriminating experiment design); `researcher-in-the-loop` absorbs `high-impact-checkpoint`; `myst-expert` absorbs `myst-ci`; `mystmd-plugin-dev` absorbs `interactive-figures`. Also fixed a garbled sentence in the seed guidance and dropped the project-specific `.claude-work` completion-note path.
...
- **Figure craft & interpretation layer** (67 → 70 skills), completing the figure lifecycle:
  - `astro-plotting-craft` (Communicate) — author publication-grade astrophysics plots in the default house style: the jaxstroviz theme + figure helpers as source of truth, seaborn perceptually-uniform colormaps (`mako`/`vlag`), CVD-safe categorical palettes with color×marker redundancy, log/linear axis discipline, LaTeX-not-unicode labels with CGS/solar units, uncertainty and overlay handling. Ships co-located references (`house-style.md`, distilled `seaborn-plotting-reference.md`).
